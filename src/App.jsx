...
const App = () => {
  ...
  const currentNotes = dailyNotes.find(n => n.date === selectedDate)?.notes || '';

  const stats = {
    total: filteredTasks.length,
    completed: filteredTasks.filter(t => t.completed).length,
    remaining: filteredTasks.filter(t => !t.completed).length,
    percentage: filteredTasks.length > 0 ? Math.round((filteredTasks.filter(t => t.completed).length / filteredTasks.length) * 100) : 0,
    totalEstimatedMinutes: filteredTasks.reduce((acc, t) => acc + (Number(t.estimatedMinutes) || 0), 0)
  };
